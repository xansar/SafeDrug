...
def combine_process(med_pd, diag_pd, pro_pd):
    ...
    #     old_data['ICD9_CODE_Len'] = old_data['ICD9_CODE'].map(lambda x: len(x))
...
def get_ddi_matrix(records, med_voc, ddi_file):
    ...
    # ddi_most_pd = pd.DataFrame(columns=['Side Effect Name'], old_data=['as','asd','as'])
...
if __name__ == "__main__":
    ...
    med_file = '~/old_data/physionet.org/files/mimiciii/1.4/PRESCRIPTIONS.csv'
    diag_file = '~/old_data/physionet.org/files/mimiciii/1.4/DIAGNOSES_ICD.csv'
    procedure_file = '~/old_data/physionet.org/files/mimiciii/1.4/PROCEDURES_ICD.csv'
    ...
    # create ehr sequence old_data
    records = create_patient_record(data, diag_voc, med_voc, pro_voc)
    print("obtain ehr sequence old_data")
