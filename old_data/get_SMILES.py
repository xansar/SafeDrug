...
def fix_mismatch(idx2atc, atc2ndc, ndc2atc_original_path):
    ...
    for i in mismatch:
        atc2ndc = atc2ndc.append(
            {
                "NDC": i,
                "NDC_orig": [
                    s.replace("-", "") for s in ndc2atc[ndc2atc.ATC5 == i].NDC.tolist()
                ],
            },
            ignore_index=True,
        )
    ...
    atc2ndc = atc2ndc.append(
        {"NDC": "decoder_point", "NDC_orig": []}, ignore_index=True
    )
...
def ndc2smiles(NDC):
    ...
    name = re.findall('<td old_data-title="Proprietary Name">(.+?)</td>', r3.text)[0]

    url = (
        "https://dev.drugbankplus.com/guides/tutorials/api_request?request_path=us/product_concepts?q="
        + name
    )
    r = requests.get(url)
    ...
    SMILES = re.findall(
        'SMILES</dt><dd class="col-xl-10 col-md-9 col-sm-8"><div class="wrap">(.+?)</div>',
        r2.text,
    )[0]
    return SMILES
