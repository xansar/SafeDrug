...
@author: xansar
@software: PyCharm
@file: processing_4.py
@time: 2023/7/18 16:13
...
def filter_1000_most_pro(pro_pd):
    pro_count = pro_pd.groupby(by=['icd_code']).size().reset_index().rename(columns={0: 'count'}).sort_values(
        by=['count'], ascending=False).reset_index(drop=True)
...
def combine_process(med_pd, diag_pd, pro_pd):
    ...
    pro_pd = pro_pd.groupby(by=['subject_id', 'hadm_id'])['icd_code'].unique().reset_index().rename(
